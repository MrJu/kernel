...
static int foo_open(struct inode *inode, struct file *filp)
{
	...
	foo = container_of(inode->i_cdev, struct foo_device, cdev);

	if (!atomic_dec_and_test(&foo->atomic)) {
		atomic_inc(&foo->atomic);
		return -EBUSY;
	}
