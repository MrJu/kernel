...
static int task_func(void *data)
{
	...
	while (!kthread_should_stop()) {
		trace_foo_bar("atomic", count / 2,
				array, current->cpus_ptr);
		trace_foo_bar_with_cond("atomic", count);
		trace_foo_bar_with_fn("atomic", count);
		trace_foo_with_template_simple("atomic", count);
		trace_foo_with_template_cond("atomic", count);
		trace_foo_with_template_fn("atomic", count);
		trace_foo_with_template_print("atomic", count);
		count++;
		count %= 16;
