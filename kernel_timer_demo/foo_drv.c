...
static int foo_show(struct seq_file *m, void *v) {
	struct foo_device *foo = m->private;
	seq_printf(m, "callback_count: %u\n", foo->callback_count);
	return 0;
}

static int foo_open(struct inode *inode, struct  file *file) {
	struct foo_device *foo = container_of(inode->i_cdev, struct foo_device, cdev);
	return single_open(file, foo_show, foo);
}
