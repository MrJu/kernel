...
/* try to comment get_cpu and put_cpu then see the performance */
static int task_func(void *data)
{
	struct cpumask mask;
	struct task_info *task_info = data;
	__sched_setaffinity
		= (long (*)(pid_t, const struct cpumask *)) 0x801509c4;
	...
	while (!kthread_should_stop()) {
		...
		printk("%s, id %d, pid %d, cpu id %d, preempt_count %d start\n",
			current->comm, task_info->id, current->pid,
			raw_smp_processor_id(), preempt_count());

		if (task_info->id == 0) {
			mdelay(DELAY_IN_MSEC);
		} else {
			mdelay(DELAY_IN_MSEC * 2);
		}

		printk("%s, id %d, pid %d, cpu id %d, preempt_count %d end\n",
			current->comm, task_info->id, current->pid,
			raw_smp_processor_id(), preempt_count());
