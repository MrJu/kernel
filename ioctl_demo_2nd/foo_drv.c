...
static long foo_unlocked_ioctl(struct file *file,
		unsigned int cmd, unsigned long arg)
{
	return foo_ioctl(file, cmd, arg);
}

#if defined(CONFIG_64BIT)
static long foo_compat_ioctl(struct file *file,
		unsigned int cmd, unsigned long arg)
{
	return foo_ioctl(filem cmd, (unsigned long)compat_ptr(arg));
}
#endif
...
#if defined(CONFIG_64BIT)
	.compat_ioctl = foo_unlocked_ioctl,
#endif
	.release = foo_release,
};
