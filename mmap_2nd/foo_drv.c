...
static int vm_ops_fault(struct vm_fault *vmf)
{
	struct page *page;
	...
	page = virt_to_page(info->buf + (vma->vm_pgoff << PAGE_SHIFT));
	get_page(page);
...
static int foo_open(struct inode *inode, struct file *filp)
{
	...
	info->order = PAGE_ORDER;

	info->buf = (void*)__get_free_pages(
			GFP_KERNEL | __GFP_ZERO, info->order);
	if (!info->buf) {
		kfree(info);
		return -ENOMEM;
	}

	sprintf(info->buf, "%s(): process: %s, pid: %d\n",
...
{
	struct foo_device *foo = filp->private_data;
	struct mmap_info *info = foo->info;

	free_pages((unsigned long)info->buf, info->order);
	kfree(info);
